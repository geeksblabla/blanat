...
void *nta_tarikh_ana_joghrafiya(void *arg) {
    s_thread_ctx *ctx = (s_thread_ctx *)arg;
    char *begin = ctx->begin;
    char *end = ctx->end;
...
void process_blk(char *begin, char *end, uint8_t proc, char *ans) {
    if (proc && *(begin - 1) != '\n')
        while (*(begin++) != '\n') {
        }

    pthread_t threads[MAX_THREADS];
    s_thread_ctx threads_ctx[MAX_THREADS];
    const uint64_t BLK_SIZE = MAX(4096L, (end - begin) / MAX_THREADS);
    uint32_t threads_cnt = 0;
    for (uint8_t tid = 0; begin < end && tid < MAX_THREADS; ++tid, ++threads_cnt) {
        s_thread_ctx *ctx = threads_ctx + tid;
        ctx->begin = begin;
        ctx->end = MIN(begin + BLK_SIZE, end);
        while (ctx->end < end && *(ctx->end) != '\n') ++(ctx->end);
        pthread_create(&threads[tid], NULL, nta_tarikh_ana_joghrafiya, (void *)ctx);
        begin = ctx->end + 1;
    }

    for (int tid = 0; tid < threads_cnt; ++tid) pthread_join(threads[tid], NULL);

    uint64_t prices_sum_per_city[CITIES_NUM] = {0};
    uint16_t prices[CITIES_NUM][PRODUCTS_NUM] = {0};

    for (uint8_t cid = 0; cid < CITIES_NUM; ++cid) {
        for (uint8_t tid = 0; tid < threads_cnt; ++tid) {
            prices_sum_per_city[cid] += threads_ctx[tid].prices_sum_per_city[cid];
            for (uint8_t pid = 0; pid < PRODUCTS_NUM; ++pid) {
                if ((threads_ctx[tid].prices[cid][pid] < prices[cid][pid] && threads_ctx[tid].prices[cid][pid]) ||
                    !prices[cid][pid])
                    prices[cid][pid] = threads_ctx[tid].prices[cid][pid];
            }
        }
    }
...
int main(void) {
    ...
    for (uint32_t proc = 0; offset < sb.st_size && proc < MAX_PROCESSES; ++proc, ++procs_cnt) {
        ...
        pids[proc] = fork();
        if (!pids[proc]) {
            process_blk(fbegin + offset, fbegin + offset + size, proc, ans + (ANS_SIZE * proc));
            exit(EXIT_SUCCESS);
        } else if (pids[proc] < 0)
            exit(EXIT_FAILURE);
        offset += size;
